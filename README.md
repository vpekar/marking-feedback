...
(4) Create a repository of previous comments: This should be a plain-text file, where the name of the rubric section appears on a separate line and is preceded with a "#", and past comments relating to this section appear below, one line per comment. See the example file included with the plugin on github. Note that shorter comments will be easier to work with, as the autocompletion menu has a limited width. Save the file anywhere on your computer, e.g. under "C:\Users\<username>\comments_repository.txt".
